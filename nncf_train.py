...
import nncf_test  # import test.py to get mAP after each epoch
...
def train(hyp, opt, device, tb_writer=None, wandb=None):
    ...
    nncf_config=NNCFConfig.from_json('yolov5_nncf.json')
    ...
    for epoch in range(start_epoch, epochs):  # epoch ------------------------------------------------------------------
        ...
        compress_ctrl.scheduler.epoch_step()
...
#                     ema.update(model)
        ...
        if rank in [-1, 0]:
            ...
            if not opt.notest or final_epoch:  # Calculate mAP
                results, maps, times = nncf_test.test(opt.data,
            ...
            None_best_fitness=0
            PARTIAL_best_fitness=0
            FULL_best_fitness=0
            level=compress_ctrl.compression_level
            if level=='None':
                if fi>None_best_fitness:
                    torch.save(
                        model.state_dict(),

                    'None_best.pt')
                    None_best_fitness=fi
            elif level=='PARTIAL':
                if fi > PARTIAL_best_fitness:
                    torch.save(
                        model.state_dict(),

                   'PARTIAL_best.pt')
                    PARTIAL_best_fitness=fi
            else:
                if fi>FULL_best_fitness:
    ...
    if rank in [-1, 0]:
        ...
        if opt.data.endswith('coco.yaml') and nc == 80:  # if COCO
            for m in (last, best) if best.exists() else (last):  # speed, mAP tests
                results, _, _ = nncf_test.test(opt.data,
                                          batch_size=batch_size * 2,
                                          imgsz=imgsz_test,
                                          conf_thres=0.001,
                                          iou_thres=0.7,
